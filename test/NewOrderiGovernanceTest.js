const OpenGovernance = artifacts.require("NewOrderGovernance");
 ...
 */
contract("NewOrderGovernance", function (accounts) {
    //const wallet = accounts[0]
    // const walletTo = accounts[1]
    let token //: Contract;
    const tokenName = "OpenGovernanceTestingToken";
  ...
  afterEach(async() => {
    ...
    beforeEach(async () => {
        token = await OpenGovernance.new(tokenName, "ODGTT", initial_tokens, accounts[0]);
    ...
    it('transferFrom: _transfer from zero address not allowed.', async () => {
      ...
      let threw = false
      try {
        await token.transferFrom('0x0000000000000000000000000000000000000000', accounts[ 2 ], 20, { from: accounts[ 1 ] })
      } catch (e) {
        threw = true
      }
    ...
    it('transferFrom: _transfer to zero address not allowed.', async () => {
      ...
      let threw = false
      try {
        await token.transferFrom(accounts[ 2 ], '0x0000000000000000000000000000000000000000',  20, { from: accounts[ 1 ] })
      } catch (e) {
        threw = true
      }
    ...
    it('transferFrom: _transfer can not exceed balance of sender address.', async () => {
      ...
      let threw = false
      try {
        await token.transferFrom(accounts[ 2 ], accounts[ 0 ],  20, { from: accounts[ 1 ] })
      } catch (e) {
        threw = true
      }
